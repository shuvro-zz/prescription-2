...
use DateTime;
use Session;
class Reportm extends Mailable
{
    ...
    public function build()
    {
        ...
        $data['get_month_value']  = \App\model\Products::with(['purchase_details'=>function($q) use ($month){
        ...
        $data['get_sell_details']  = \App\model\Products::with(['sell_details'=>function($q) use ($month){
            $q->where('created_at','like',''.$month.'%');
            $q->orderBy('created_at','Desc');
        }]) ->get();
        ...
        return $this->from($data['company_info'][0]->email)
                ->subject($month.'-মাসের ক্রয়ের সম্পূর্ণ তথ্য')
            ->view('pos.pages.reports.monthly_purchase_pdf',$data);
    }
}