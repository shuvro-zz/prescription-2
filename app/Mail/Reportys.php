...
use DateTime;
use Session;
class Reportys extends Mailable
{
    ...
    $value=Session::all();
       ...
       $year=$this->inputs;
        $prev_year = $year - 1;
//            $this->get_year_report($year);
             $data['get_year_value']  = \App\model\Products::with(['sell_details'=>function($q) use ($year){
         ...
         $data['get_sell_details']  = \App\model\Products::with(['purchase_details'=>function($q) use ($year){
             $q->where('created_at','like',''.$year.'%');
             $q->orderBy('created_at','Desc');
         }]) ->get();
        ...
        return $this->from($data['company_info'][0]->email)
                ->subject($year.'-এর বিক্রয়ের বাৎসরিক প্রতিবেদন')
            ->view('pos.pages.reports.yearly_sell_pdf',$data);
    }
}